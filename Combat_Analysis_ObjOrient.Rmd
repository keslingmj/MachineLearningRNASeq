...
I'm making a few modifications only in the next chunk in order to 
switch to using the ComBat dataset.  2020-06-27
```{r}
...
This is the exact sample Principal Components plot as above, except that I'm only
going to color the TCGA points that are from the same patient--one healthy and one tumor.
```{r, fig.height=6, fig.width=8, dpi=300}
...
```
We can see that while U Pittsburg has many more healthy samples than tumors, the model correctly discriminates between U Pittsburg samples by whether they are actually healthy (53 samples) or tumor (22 samples).
