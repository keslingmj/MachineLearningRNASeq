...
# grab all TCGA IDs already used


# initialize six tumor stage dataframes
stageOne <- data.frame(tcgaID=character(), stage=character())
...
processTumorStage <- function(df){  #expects  1 row of 2 col df: 
   ...
   for(id in unlist(strsplit(df[1], ";"))){
      if(grepl("01[ABZ]$", id)){
         ...
         #print(c(id, tumor_stage))
         if(grepl("stage i[ab]", stage)){stageOne <<- rbind(stageOne, cbind(id, stage))}
      }
   }
}
#processTumorStage(TCGA_ID_Stage[1,])
