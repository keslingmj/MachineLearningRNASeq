...
weightedTrimmedMean <- function(refSample, testSample, refName, testName, df, refSmpUnscaled){
   ...
   print(c("header genes4norm", head(genes4norm)))
   #########
   ...
   testSelectNat <- df[testSampleName,genes4norm]
   
   print(c("name equality", head(sort(names(refSelectNat))), head(sort(names(testSelectNat)))))
   
   ######## THESE ARE NOT IN THE SAME ORDER UNLESS THEY ARE SORTED.  VERY IMPORTANT
   ######## REFSELECTNAT AND TESTSELECTNAT
   
   # print(c("scale of vects", head(refSelectNat), head(testSelectNat)))
   # looks okay
   ...
   #print(c("Nr & Nk", Nr, Nk))
   #print(c("is.na", any(is.na(refSelectNat)), any(is.na(testSelectNat))))
   # looks okay
...
i <- 1
for(trainSampleName in samples[1:2]){
   trainSample <- toilTrainScaled[i,]
   scalingFactor <- weightedTrimmedMean(refSample, trainSample, refName, trainSampleName, toilTrainFilter,
                                        refSampleUnscaled)
...
toilTestScaled <- scaleTestSamples(toilTestFiltNat, logged=TRUE)
...
for(testSampleName in samples){         # some of the passed variables were created in training set
   ...
   scalingFactor <- weightedTrimmedMean(refSample, testSample, refName, testSampleName, toilTestFiltNat, refSampleUnscaled)
   ScalFact <- c(ScalFact, scalingFactor)
   i = i+1
}
ScalFact <- unlist(ScalFact)
