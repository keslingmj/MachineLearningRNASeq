...
head(colnames(toilData))
toilSampleNames <- as.character(toilSampleNames)

stageOneIDs <- stageOne %>% filter(novel=="unused") %>% select(id) %>% 
   {gsub("[ABZC]$", "", stageOne$id, perl=TRUE)} %>% unique()   # 188 long

stageTwoIDs <- stageTwo %>% filter(novel=="unused") %>% select(id) %>% 
   {gsub("[ABZC]$", "", stageTwo$id, perl=TRUE)} %>% unique()   # 632 long

stageThreeIDs <- stageThree %>% filter(novel=="unused") %>% select(id) %>% 
   {gsub("[ABZC]$", "", stageThree$id, perl=TRUE)} %>% unique()   # 251 long

stageFourIDs <- stageFour %>% filter(novel=="unused") %>% select(id) %>% 
   {gsub("[ABZC]$", "", stageFour$id, perl=TRUE)} %>% unique()   # 20 long

stageHealthyIDs <- stageHealthy %>% filter(novel=="unused") %>% select(id) %>% 
   {gsub("[ABZC]$", "", stageHealthy$id, perl=TRUE)} %>% unique()   # 1161 long ??????
### NOTE THAT THERE IS A FULL EXPANSION OF ALL TISSUE SAMPLES DERIVED FROM A PERSON
### IN THE DATASET, BUT MANY OF THESE SAMPLES WILL NOT BE PART OF THE TCGA DATASET

### ALSO NOTE THAT I'M GETTING A LOT MORE SAMPLES THAN EXPECTED BECAUSE I'M NOT 
### IMPOSING THE WANG QA FILTERS, AND I'M GRABBING ALL SAMPLES--but some of that
### is controlled when matching up with toilData dataframe:
### I'm also transposing the dataframes here as well:
stageOneDF <- t(toilData[,which(toilSampleNames %in% stageOneIDs)])          # 182 smpls
stageTwoDF <- t(toilData[,which(toilSampleNames %in% stageTwoIDs)])          # 622
stageThreeDF <- t(toilData[,which(toilSampleNames %in% stageThreeIDs)])      # 250
stageFourDF <- t(toilData[,which(toilSampleNames %in% stageFourIDs)])        #  20
stageHealthyDF <- t(toilData[,which(toilSampleNames %in% stageHealthyIDs)])  # 113
...
stageHealthNat <- (2^stageHealthyDF)-1    # natural scale
zeroExpGenes <- which(apply(stageHealthyDF, 2, function(x) quantile(x)[4]) < 0.2)    
stageHealthFilter <- stageHealthNat[,-zeroExpGenes]
```
### 2. Select a Reference Sample Amongst Test Samples for Scaling Data
```{r}
