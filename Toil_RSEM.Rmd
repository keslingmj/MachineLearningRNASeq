...
coefsToilNonZero <- coefsToil[idx]                       # fixed
...
# A BUG PERSISTS REGARDING RELATIVE SCALING OF TUMOR VS HEALTHY
# ANOTHER PLOT BELOW DOESN'T HAVE THIS ISSUE
...
As it appears that PC2 separates healthy / tumor samples better than PC1, I'll plot the gene predictors best or worst correlated with PC2
...
# FOR KEYNOTE PRESENTATION -- CHANGE fig.height to 56.
# grid.arrange(ls[[1]], ls[[2]], ls[[3]], ls[[4]], ls[[5]], ls[[6]],
...
# FOR MARKDOWN DOC:
grid.arrange(ls[[6]], ls[[8]], ls[[9]], ls[[15]], ls[[21]], ls[[22]],
             ls[[40]], ls[[42]], ncol=2)
```
...
# define geneColors based on absolute value of model coefficient:
genesDendOrder <- c("NTNG2", "FNDC1", "BGN", "FAM89B", "CHPF2", "LIMK1", "ZNF668", "TTYH3",
  "ATP6V0B", "TBL2", "CTD", "PAFAH1B3", "MAZ", "CDK5", "H3F3A", "SRP9",
  "ABCG1", "TRBV11", "RP5", "ABCA5", "KLHL29", "USP44", "KCNJ2", "NDRG2",
  "CLDN8", "CNTNAP3P2", "AC093609", "MIR497HG", "LMOD1", "NKAPL", "SDPR",
  "ADAMTS5", "ARHGAP20", "SLC17A7", "KLF15", "PDK4", "TINAGL1", "SRPX",
  "EDNRB", "RILP", "FXYD1", "GABARAPL1", "(Intercept)")
#sort(order(genesDendOrder, coefsDF$hgnc_symbol[2:dim(coefsDF)[1]]))
# coefsDF %>% dplyr::arrange(hgnc_symbol[order(coefsDF$hgnc_symbol, genesDendOrder)])   # the one we need for the code

coefsDF2 <- coefsDF
rownames(coefsDF2) <- coefsDF2$hgnc_symbol
pred42coefsOrdered <- coefsDF2[genesDendOrder,]$coefs
pred42coefsOrdered <- pred42coefsOrdered[1:(length(pred42coefsOrdered)-1)] # rm intercept
range01 <- function(x)(x-min(x))/diff(range(x))
# colorRampPalette(brewer.pal(11, "BrBG"))[range01(pred42coefsOrdered)]     # sort this out
# colorRampPalette(brewer.pal(11, "BrBG"))[cut(range01(pred42coefsOrdered), breaks=11)] 
geneCols <- palette(brewer.pal(11, "RdBu"))[cut(range01(pred42coefsOrdered), breaks=11, labels=FALSE)]  # need to convert to integers 1-11, not ranges.

#hmcol <- colorRampPalette(brewer.pal(9, "GnBu"))(100)    # set coloring heat map
# cols <- palette(brewer.pal(3, "Set1"))[samples]
# geneCols <- colorRampPalette(brewer.pal(11, "BrBG"))[pred42coefsOrdered]

# narrow the color dynamic range by manually modifying brewer.pal(11, "PRGn")
hmcol <- colorRampPalette(c("#40004B", "#40004B", "#40004B", "#762A83", "#9970AB", 
  "#F7F7F7",
  "#5AAE61", "#1B7837", "#00441B", "#00441B", "#00441B"))

heatmap.2(as.matrix(predictors42scaled), col=hmcol, trace="none", cexRow = 0.15, cexCol = 0.5,  
...
stageTwoDF <- read.csv("stageTwoTest.txt", header=TRUE, sep="\t",
           stringsAsFactors = FALSE)
stageThreeDF <- read.csv("stageThreeTest.txt", header=TRUE, sep="\t",
           stringsAsFactors = FALSE)
stageFourDF <- read.csv("stageFourTest.txt", header=TRUE, sep="\t",
           stringsAsFactors = FALSE)
stageHealthyDF <- read.csv("stageHealthyTest.txt", header=TRUE, sep="\t",
           stringsAsFactors = FALSE)
# REPEAT
