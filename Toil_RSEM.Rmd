...
#### 1. The first step is to remove genes whose expression is very close to zero.  For example, We know that there are about 9103 genes that are never expressed, but there are over 25000 genes whose 75th quantile-level expression is under 2^(0.2) - 1 = 0.14 counts. We really don't want to deal with those genes in selecting a reference sample, etc.
```{r}
...
weightedTrimmedMean <- function(refSample, testSample, refName, testName, testSmpUnscaled, refSmpUnscaled){
   ...
   #print(c("lengths", length(refSampleFilt), length(testSampleFilt)))
   #print(head(setdiff(names(refSampleFilt), names(testSampleFilt))))
