...
#################################### TEST
wangTrainNorm2 <- t(apply(wangTrainMatrix, 1, function(x){x / cSTD}))
...
wangTrainNorm2 <- cbind(wangTrainNorm2, zeros)
...
################################### TEST
wangTestNorm2 <- t(apply(wangTestMatrix, 1, function(x){x / cSTD}))
...
wangTestNorm2 <- cbind(wangTestNorm2, zeros)
wangTestNorm2 <- wangTestNorm2[,colnames(wangTrainNorm)]
wangTestNorm <- wangTestNorm[,colnames(wangTrainNorm)]  # reorder columns
# for consistency with wangTrainNorm
...
###############
### TESTING LASSO ON UN-NORMALIZED DATA
...
which(coef(cv.lasso)!= 0) # 37 coefs
which(coef(cv.lasso2)!= 0) # 28 coefs
which(coef(cv.lasso2)!= 0) %in% which(coef(cv.lasso)!=0) # almost a subset
plot(cv.lasso)
```
