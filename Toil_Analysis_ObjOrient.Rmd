...
```
#### Remove Non-Predictor Genes from Filtered Test Data
...
```
These results show that I have absolutely no sensitivity nor specificity.
...
pcDat <- data.frame(cbind(PCs$x[,1:10], outcome=as.factor(train4stage$outcome)))
...
trainPred <- ifelse(predict(svmfit, pcDat %>% dplyr::select(-outcome)) < 1.5, 1, 2) # only works with outcome var!
